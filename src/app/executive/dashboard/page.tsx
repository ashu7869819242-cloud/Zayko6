        ...
        return (
            <div className="min-h-screen bg-zayko-900 flex items-center justify-center">
                <div className="text-center">
                    ...
                    <p className="text-zayko-400 text-sm">Loading executive analytics...</p>
                </div>
            </div>
        );
    }
        ...
        <div className="min-h-screen bg-zayko-900 pb-12">
            ...
            <div className="bg-zayko-800 border-b border-zayko-700 px-6 py-4">
                <div className="max-w-7xl mx-auto flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h1 className="text-lg font-display font-bold text-white">Executive Overview</h1>
                            <p className="text-xs text-indigo-400">Business Intelligence Dashboard</p>
                        </div>
                    </div>
                    <button
                        onClick={() => { setLoading(true); fetchAll(); }}
                        className="px-4 py-2 bg-indigo-500/20 text-indigo-400 rounded-xl text-sm font-semibold hover:bg-indigo-500/30 transition-all"
                    >
                        🔄 Refresh
                    </button>
                </div>
            ...
            <div className="max-w-7xl mx-auto p-6">
                ...
                <div className="grid grid-cols-2 lg:grid-cols-5 gap-4 mb-8 animate-fade-in">
                    {[
                        { label: "Total Revenue", value: `₹${(financial?.totalRevenue || 0).toLocaleString()}`, icon: "💰", color: "text-emerald-400" },
                        { label: "Today Revenue", value: `₹${(financial?.todayRevenue || 0).toLocaleString()}`, icon: "📅", color: "text-blue-400" },
                        { label: "Total Orders", value: orders?.totalOrders?.toLocaleString() || "0", icon: "📦", color: "text-purple-400" },
                        { label: "Avg Order", value: `₹${orders?.avgOrderValue || 0}`, icon: "🎯", color: "text-gold-400" },
                        { label: "Repeat Rate", value: `${orders?.repeatRate || 0}%`, icon: "🔁", color: "text-indigo-400" },
                    ].map((card) => (
                    ...
                    <div className="bg-gradient-to-br from-indigo-500/10 to-violet-500/5 border border-indigo-500/20 rounded-2xl p-6 mb-8 animate-slide-up">
                        <div className="flex items-center gap-3 mb-4">
                            ...
                            <div>
                                ...
                                <p className="text-[10px] text-indigo-400 uppercase tracking-wider font-bold">Auto-Generated Analysis</p>
                            </div>
                        </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
                    ...
                    <div className="lg:col-span-2 bg-zayko-800/50 border border-zayko-700 rounded-2xl p-6 animate-slide-up">
                        <h3 className="text-sm font-display font-bold text-white mb-4 flex items-center gap-2">
                            <span className="text-lg">📈</span> Revenue Trend (30 Days)
                        </h3>
                        <div className="h-64">
                    ...
                    <div className="bg-zayko-800/50 border border-zayko-700 rounded-2xl p-6 animate-slide-up">
                        <h3 className="text-sm font-display font-bold text-white mb-4 flex items-center gap-2">
                            <span className="text-lg">💳</span> Payment Split
                        </h3>
                        <div className="h-48">
                            <ResponsiveContainer width="100%" height="100%">
                                <PieChart>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
                    ...
                    <div className="bg-zayko-800/50 border border-zayko-700 rounded-2xl p-6 animate-slide-up">
                        <h3 className="text-sm font-display font-bold text-white mb-1 flex items-center gap-2">
                            <span className="text-lg">⏰</span> Hourly Order Distribution
                        </h3>
                        <p className="text-[10px] text-zayko-500 mb-4">Peak: {orders?.peakHour} ({orders?.peakHourOrders} orders)</p>
                        <div className="h-52">
                            <ResponsiveContainer width="100%" height="100%">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 animate-fade-in">
                    {[
                        { label: "Razorpay Revenue", value: `₹${(financial?.razorpayRevenue || 0).toLocaleString()}`, color: "text-indigo-400", bg: "bg-indigo-500/10 border-indigo-500/20" },
                        { label: "Wallet Revenue", value: `₹${(financial?.walletRevenue || 0).toLocaleString()}`, color: "text-violet-400", bg: "bg-violet-500/10 border-violet-500/20" },
                        { label: "This Month", value: `₹${(financial?.monthRevenue || 0).toLocaleString()}`, color: "text-emerald-400", bg: "bg-emerald-500/10 border-emerald-500/20" },
                    ].map((card) => (
                        <div key={card.label} className={`rounded-2xl p-5 border ${card.bg}`}>
                            <p className="text-[10px] text-zayko-400 uppercase tracking-wider font-bold mb-1">{card.label}</p>
