...
export default function StockInventoryPage() {
    ...
    const fetchInventory = useCallback(async () => {
        try {
            ...
            const res = await fetch(`/api/stock/inventory?${params}`, { headers: getHeaders() });
    ...
    const fetchLogs = async (itemId: string) => {
        try {
            const res = await fetch(`/api/stock/inventory/logs?itemId=${itemId}&limit=30`, { headers: getHeaders() });
            const data = await res.json();
    ...
    const fetchAnalytics = async () => {
        try {
            const res = await fetch("/api/stock/inventory/analytics", { headers: getHeaders() });
            const data = await res.json();
    ...
    const fetchRecipeMappings = async (menuItemId: string) => {
        try {
            const res = await fetch(`/api/stock/inventory/recipe-mapping?menuItemId=${menuItemId}`, { headers: getHeaders() });
            const data = await res.json();
            if (data.success) {
    ...
    const handleEditItem = async () => {
        ...
        setSaving(true);
        try {
            const res = await fetch(`/api/stock/inventory/${editItem.id}`, {
    ...
    const handleDeleteItem = async (id: string) => {
        ...
        try {
            const res = await fetch(`/api/stock/inventory/${id}`, { method: "DELETE", headers: getHeaders() });
            const data = await res.json();
    ...
    const handleStockAdjust = async () => {
        ...
        setSaving(true);
        try {
            const res = await fetch("/api/stock/inventory/stock-adjust", {
    ...
    const handleSaveRecipeMapping = async () => {
        ...
        setSaving(true);
        try {
            const res = await fetch("/api/stock/inventory/recipe-mapping", {
                method: "POST", headers: getHeaders(),
    ...
    const handleExportCSV = async () => {
        try {
            const token = localStorage.getItem("stockManagerToken");
            const res = await fetch("/api/stock/inventory/export", { headers: { Authorization: `Bearer ${token}` } });
                ...
                <div className="max-w-7xl mx-auto flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-emerald-400">Track ingredients, stock & recipes</p>
                        </div>
                    </div>
                    <div className="flex items-center gap-2 overflow-x-auto pb-2 sm:pb-0 w-full sm:w-auto scrollbar-hide">
                        ...
                        <button onClick={() => { setShowAddDialog(true); resetForm(); }} className="py-2 px-4 text-sm whitespace-nowrap rounded-xl font-semibold text-white bg-gradient-to-r from-emerald-600 to-emerald-500 hover:from-emerald-500 hover:to-emerald-400 transition-all shadow-lg hover:shadow-emerald-500/25">
                            + Add Item
                        </button>
                    </div>
                </div>
            ...
            <div className="max-w-7xl mx-auto p-6 space-y-6">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 animate-fade-in">
                    {[
                        ...
                        { label: "Total Value", value: `₹${summary.totalValue.toLocaleString()}`, icon: "💰", color: "text-emerald-400" },
                    ...
                    <>
                        <div className="flex flex-col sm:flex-row gap-3 animate-slide-up">
                            <input type="text" placeholder="Search items..." value={search}
                                ...
                                className="flex-1 bg-zayko-800 border border-zayko-700 rounded-xl px-4 py-2.5 text-sm text-white focus:outline-none focus:border-emerald-500 transition-colors"
                            />
                            <select value={categoryFilter} onChange={(e) => setCategoryFilter(e.target.value)}
                                className="bg-zayko-800 border border-zayko-700 rounded-xl px-4 py-2.5 text-sm text-white focus:outline-none focus:border-emerald-500"
                            >
                                        ...
                                        <tbody>
                                            {items.map((item) => (
                                                <tr key={item.id} className="border-b border-zayko-700/50 hover:bg-zayko-800/80 transition-colors">
                                                    ...
                                                    <td className="px-4 py-3 text-right text-emerald-400 font-semibold">₹{(item.currentStock * item.costPerUnit).toLocaleString()}</td>
                            ...
                            <>
                                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                    <div className="bg-zayko-800/50 border border-zayko-700 rounded-2xl p-4">
                                        ...
                                        <p className="text-2xl font-display font-bold text-emerald-400">₹{analytics.totalValue.toLocaleString()}</p>
                                    </div>
                                    <div className="bg-zayko-800/50 border border-zayko-700 rounded-2xl p-4">
                                        ...
                                        <div className="space-y-2">
                                            {analytics.mostUsedIngredients.map((ing, i) => (
                                                <div key={i} className="flex items-center justify-between py-2 border-b border-zayko-700/50 last:border-0">
                                                    ...
                                                    <span className="text-emerald-400 font-mono">{ing.totalDeducted} {ing.unit}</span>
                                                </div>
                                            ))}
                                        </div>
                                    )}
                                                ...
                                                <tbody>
                                                    {analytics.purchaseSuggestions.map((s, i) => (
                                                        <tr key={i} className="border-b border-zayko-700/50">
                                                            ...
                                                            <td className="py-2 px-3 text-right text-emerald-400">₹{s.estimatedCost.toLocaleString()}</td>
                ...
                {activeTab === "recipes" && (
                    <div className="space-y-6 animate-fade-in">
                        <div className="bg-zayko-800/50 border border-zayko-700 rounded-2xl p-6">
                            ...
                            <select value={selectedMenuItemId}
                                ...
                                className="w-full bg-zayko-900 border border-zayko-700 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-emerald-500 mb-4"
                            >
                                ...
                                <div className="space-y-3">
                                    ...
                                    {recipeIngredients.map((ing, idx) => (
                                        <div key={idx} className="flex items-center gap-2 p-3 bg-zayko-900/50 rounded-xl border border-zayko-700">
                                            ...
                                            <input type="number" min="0" step="0.1" placeholder="Qty" value={ing.quantityRequired || ""}
                                    ...
                                    {recipeIngredients.length > 0 && (
                                        <button onClick={handleSaveRecipeMapping} disabled={saving} className="py-2.5 px-6 text-sm w-full mt-2 rounded-xl font-semibold text-white bg-gradient-to-r from-emerald-600 to-emerald-500 hover:from-emerald-500 hover:to-emerald-400 transition-all shadow-lg hover:shadow-emerald-500/25 disabled:opacity-50">
                    ...
                    <div className="bg-zayko-800 border border-zayko-700 rounded-2xl p-6 w-full max-w-md max-h-[90vh] overflow-y-auto animate-scale-in">
                        ...
                        <div className="space-y-3">
                            <div>
                                ...
                                <input type="text" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className="w-full bg-zayko-900 border border-zayko-700 rounded-xl px-4 py-2.5 text-sm text-white focus:outline-none focus:border-emerald-500" placeholder="e.g., Buns" />
                            </div>
                        ...
                        <div className="flex gap-3 mt-6">
                            ...
                            <button onClick={editItem ? handleEditItem : handleAddItem} disabled={saving} className="flex-1 py-2.5 text-sm rounded-xl font-semibold text-white bg-gradient-to-r from-emerald-600 to-emerald-500 hover:from-emerald-500 hover:to-emerald-400 disabled:opacity-50">{saving ? "Saving..." : editItem ? "Update" : "Add Item"}</button>
                        </div>
                    </div>
                </div>
            )}
                ...
                <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                    <div className="bg-zayko-800 border border-zayko-700 rounded-2xl p-6 w-full max-w-md animate-scale-in">
                        ...
                        <div className="flex gap-3 mt-6">
                            ...
                            <button onClick={handleStockAdjust} disabled={saving} className="flex-1 py-2.5 text-sm rounded-xl font-semibold text-white bg-gradient-to-r from-emerald-600 to-emerald-500 hover:from-emerald-500 hover:to-emerald-400 disabled:opacity-50">{saving ? "Saving..." : "Confirm"}</button>
                        </div>
                    </div>
                </div>
            )}
                                ...
                                {logs.map((log) => (
                                    <div key={log.id} className="p-3 bg-zayko-900/50 rounded-xl border border-zayko-700/50">
                                        ...
                                        <p className="text-sm text-white">
                                            {log.type === "ADD" ? "+" : "−"}{log.quantity} → <span className="font-mono text-emerald-400">{log.newStock}</span>
                                        </p>
